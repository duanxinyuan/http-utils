...
public class PutBuilder extends OkBuilder {
    ...
    public <T> PutBuilder buildPut(String url, Headers headers, Params params, T body, MediaType type) {
        if (null == params || params.size() == 0) {
            url(url).put(getRequestBody(headers, body, type));
        } else {
            url(addQueryParameter(url, params)).put(getRequestBody(headers, body, type));
        }
        return this;
    }
}
