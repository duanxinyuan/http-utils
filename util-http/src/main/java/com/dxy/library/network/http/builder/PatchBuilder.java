...
public class PatchBuilder extends OkBuilder {
    ...
    public <T> PatchBuilder buildPatch(String url, Headers headers, Params params, T body, MediaType type) {
        if (null == params || params.size() == 0) {
            url(url).patch(getRequestBody(headers, body, type));
        } else {
            url(addQueryParameter(url, params)).patch(getRequestBody(headers, body, type));
        }
        return this;
    }
