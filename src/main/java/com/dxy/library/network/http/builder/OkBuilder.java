...
public class OkBuilder extends Request.Builder {
    ...
    public static <T> OkBuilder builder(HttpSerializer httpSerializer, Method method, String url, Headers headers, Params params, T t, List<FileParam> fileParams) {
        switch (method) {
            case GET:
                //GET不支持传输Body
            ...
            case POST:
                if (null != fileParams && !fileParams.isEmpty()) {
                    ...
                    if (null == t) {
                        ...
                        return new PostBuilder(httpSerializer).buildPost(url, headers, params, t, getMediaType(headers));
                    }
                }
            case PUT:
                if (null == t) {
                    ...
                    return new PutBuilder(httpSerializer).buildPut(url, headers, params, t, getMediaType(headers));
                }
            case PATCH:
                if (null == t) {
                    return new PatchBuilder(httpSerializer).buildPatch(url, headers, params);
                } else {
                    return new PatchBuilder(httpSerializer).buildPatch(url, headers, params, t, getMediaType(headers));
                }
            case DELETE:
                //DELETE不支持传输Body
    ...
    <T> RequestBody getRequestBody(Headers headers, T t, MediaType type) {
        ...
        if (t instanceof String) {
            ...
            return RequestBody.create(type, httpSerializer.to(t));
        }
    }
